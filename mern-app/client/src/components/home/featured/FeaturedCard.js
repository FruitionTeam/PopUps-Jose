...
function FeaturedCard({ delay, title, cuisine, rating }) {
  return (
    <div
    ...
    >
      <div className="flex flex-col gap-0">
        <p className="text-2xl font-bold text-[#473837]">{title}</p>
        <p className="text-md text-[#5B4948] mt-0.4">{cuisine}</p>
      </div>
      <div className="bg-[#dbdbdb] h-[230px] w-[235px] mx-auto mt-2 items-center justify-center flex">
        image
      </div>
      <div className="mt-2 text-3xl text-yellow-300">{rating}</div>
    </div>
  );
}
